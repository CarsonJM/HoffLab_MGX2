...
rule sample2markers:
    ...
    output:
        results + "04_READ_BASED_PHYLOGENY/01_consensus_markers/{sample}/{sample}.pkl"
    log:
        results + "00_LOGS/04_sample2markers.{sample}.log"
    params:
        out_dir=results + "04_READ_BASED_PHYLOGENY/01_consensus_markers/{sample}/",
        extra_args=config["read_phylogeny"]["samples2markers_arguments"]
    conda:
        "../envs/biobakery_workflows.yml"
    container: 
        "docker://biobakery/workflows:3.0.0.a.7"
    benchmark:
        "benchmark/04_READ_BASED_PHYLOGENY/sample2markers_{sample}.tsv"
    resources:
        runtime="08:00:00",
        mem_mb="50000",
    threads:
        config["read_phylogeny"]["samples2markers_threads"]
    shell:
        """
        ...
        sample2markers.py -i {input} -o {params.out_dir} \
        --nprocs {threads} \
        --input_format sam \
        {params.extra_args}
        """
...
checkpoint strainphlan_clades:
    input:
        pkl=expand(results + "04_READ_BASED_PHYLOGENY/01_consensus_markers/{sample}/{sample}.pkl", sample=samples),
    output:
        results + "04_READ_BASED_PHYLOGENY/02_clades/strainphlan_clades"
    log:
        results + "00_LOGS/04_strainphlan_clades.log"
    params:
